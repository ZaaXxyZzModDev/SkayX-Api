...
router.post('/signup', recaptcha.middleware.verify, captchaRegister, async(req, res) => {
    ...
    if (!email || !username || !password || !confirmpassword) {
        req.flash('error_messages','Semua harus diisi sesuai ketentuan, tidak boleh kosong !');
        res.redirect('/signup');
    } else if (password != confirmpassword) {
        req.flash('error_messages',"Password tidak sama !");
        res.redirect('/signup');
    } else if(!checkpw ) {
        req.flash('error_messages',"Password harus berisi setidaknya satu nomor, satu huruf kapital dan satu huruf kecil, dan setidaknya 8 karakter tidak ada emoji dan tidak berspasi. Limit 30 karakter!");
        res.redirect('/signup');  
        ...
        req.flash('error_messages',"Sorry, kami tidak menerima akun selain gmail !, Proteksi dari temp-mail !");
        res.redirect('/signup');  
    }else{
        ...
        user.findOne({ $or: [{ email: email }, { username: username }] }, function (err, data) {
            if (err) throw err;
            if (data) {
                req.flash('error_messages',"User sudah terdaftar, silahkan login !");
                res.redirect('/signup');
            } else {
                bcryptjs.genSalt(12, (err, salt) => {
                    ...
                    bcryptjs.hash(password, salt, (err, hash) => {
                        ...
                        user({
                            ...
                            if (err) throw err;
                            req.flash('success_messages',"Akun sukses dibuat! Silahkan login");
                            res.redirect('/login');
                        });
                    })
                });
...
router.get('/send-verification-email', checkAuth, async (req, res) => {
    ...
    if (req.user.isVerified ) {
        ...
        if (check) {
        req.flash('error_messages', 'Mohon untuk tidak spam, tunggu 30 Menit !')
        res.redirect('/docs');
        }else{
        ...
        if(mail == 'error'){
            req.flash('error_messages','Error !, Lakukan lagi besok !');
            res.redirect('/docs');
        }else{
        req.flash('success_messages', 'Done ! Link expired dalam 30 Menit !')
        res.redirect('/docs');
        }
...
router.get('/verifyemail', async (req, res) => {
    ...
    if (token) {
        ...
        if (check) {
            ...
            req.flash('error_messages', 'Link telah expired atau Error')
            res.redirect('/login');
        }
    }
    } else {
...
if (userData) {
if (Cooldown) {
    req.flash('error_messages','Mohon untuk tidak spam !, tunggu 30 Menit setelah submit !');
    res.redirect('/forgot-password')
            ...
            if(mail == 'error'){
                req.flash('error_messages','Error, mohon ulangi lagi besok !');
                res.redirect('/forgot-password');
            }else{
             await resetToken({ token: token, email: email }).save();
            req.flash('success_messages','Cek emailmu untuk info lebih lanjut, mohon tunggu 30 Menit setelah submit.');
            res.redirect('/forgot-password');    
            }
           
 }
    } else {
        req.flash('error_messages','Tidak ada user terdaftar yang menggunakan email tersebut !');
        res.redirect('/forgot-password');
    }
});
...
var checkpw = resetpw.validate(password)
    ...
    if (!password || !confirmpassword || confirmpassword != password) {
        ...
        if (salt) {
            ...
            req.flash('success_messages', 'Password telah diubah')
            res.redirect('/login');
        } else {
        req.flash('error_messages',"Unexpected Error, ulangi lagi !");
        res.redirect(`/reset-password?token=${token}`);
        }
    }
});
